...
class PublicationsService {
  ...
  static Future<Map<String, dynamic>> getPublications({int page = 1, int limit = 10}) async {
    try {
      ...
      final response = await http.get(
        Uri.parse('${ApiConfig.baseUrl}/api/publicaciones?page=$page&limit=$limit'),
        headers: {
  ...
  static Future<List<dynamic>> getComments(int publicationId) async {
    try {
      ...
      final response = await http.get(
        Uri.parse('${ApiConfig.baseUrl}/api/publicaciones/$publicationId/comentarios'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
        },
  ...
  static Future<Map<String, dynamic>> createComment(
    ...
    try {
      final url = '${ApiConfig.baseUrl}/api/publicaciones/$publicationId/comentarios';
  ...
  static Future<Map<String, dynamic>> createReply(
    ...
    try {
      final url = '${ApiConfig.baseUrl}/api/publicaciones/$publicationId/comentarios/$parentCommentId/respuestas';
      ...
      if (e.toString().contains('SocketException') || e.toString().contains('TimeoutException')) {
        throw Exception('🌐 Error de conexión: Verifica tu conexión a internet y que el servidor esté funcionando. Detalles: $e');
